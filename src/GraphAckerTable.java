...
public class GraphAckerTable extends JApplet implements ActionListener {
	...
	private int ackermannValue(int m, int n, Graphics page) {
		...
		if ((h - n) == 598 || (h + 535 - n) == 598)
			setNewColor(page, (int) (Math.random() * 12) + 1);
		if (n >= 535) {
			page.drawLine(count, 600, count, h + 535 - n);
		} else
			page.drawLine(count, 600, count, h - n);
